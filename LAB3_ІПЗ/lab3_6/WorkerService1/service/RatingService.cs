...
public class RatingService
{
    ...
    public RatingService(AppDbContext context, ILogger<TrainService> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task<(bool success, string message)> AddRating(double number, int tripId)
    {
        try
        {
            var train = await _context.Trips.FirstOrDefaultAsync(t => t.Id == tripId);
            if (train == null)
            {
        ...
        try
        {
            var trips = await _context.Trips
                ...
                .Select(g => new TripRatingGetResponse
                {
                    ...
                    TrainName = g.Key.TrainName,
                    Mark = g.SelectMany(t => _context.Ratings.Where(r => r.TripId == t.Id).Select(r => r.Number)).DefaultIfEmpty().Average()  // Calculate average rating
                })
                .ToListAsync();
