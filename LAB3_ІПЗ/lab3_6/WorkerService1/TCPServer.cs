        ...
        using (var stream = client.GetStream())
        {
            ...
            try
            {
                ...
                else if (command == "add_train")
                {
                    ...
                    var (success, message) = await _trainService.CreateTrain(name, number, amountSeats);
                    var response = new
                    {
                        success = success,
                        message = message
                ...
                {
                    var name = data?["name"];
                    var busId = int.Parse(data?["trainId"]);
                ...
                else if (command == "take_a_seat_on_train")
                {
                    var userId = long.Parse(data["userId"]);
                    var busName = int.Parse(data["trainNumber"]);
                    ...
                    var (success, message) = await _trainService.TakeASeat(userId, busName, seatNumber);
                    var response = new
                    {
                        success = success,
                        message = message
